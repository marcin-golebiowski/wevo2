...
/**
 * Class for reading ENVI file containing signed short integers
 * with BSQ interleaving.
 *
 * @author Lukasz Krawiec (lmkrawiec@gmail.com)
 * @author Michal Anglart (anglart.michal@gmail.com)
 */
public final class ShortBSQFileReader extends BSQFileReader<Short> {

    /** Short size in bytes. */
    private static final int TYPESIZE = 2;
     ...
     */
    public ShortBSQFileReader(String headerFile, String dataFile)
            throws IOException {
     ...
     */
    public ShortBSQFileReader(String filename, int rows, int columns, int bands,
    ...
    protected long getNextPixelOffset(int band) {
        return this.headerOffset + ShortBSQFileReader.TYPESIZE
        * (band * this.rowsNumber * this.columnsNumber + this.position);
    }

    /** {@inheritDoc} */
    @Override
    protected List<Short> readPixel() throws IOException {
        List<Short> pixel = new ArrayList<Short>();
        if (this.buffers.get(0).remaining() < ShortBSQFileReader.TYPESIZE) {
            this.fillBuffer();
        }
        for (int i = 0; i < this.bandsNumber; i++) {
            pixel.add(this.buffers.get(i).getShort());
        }
        return pixel;
    }
