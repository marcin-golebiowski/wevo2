...
/**
 * Class for reading ENVI files containing floats with BSQ interleaving.
 *
 * @author Lukasz Krawiec (lmkrawiec@gmail.com)
 * @author Michal Anglart (anglart.michal@gmail.com)
 */
public final class FloatBSQFileReader extends BSQFileReader<Float> {

    /** Float size in bytes. */
    private static final int TYPESIZE = 4;
     ...
     */
    public FloatBSQFileReader(String headerFile, String dataFile)
            throws IOException {
     ...
     */
    public FloatBSQFileReader(String filename, int rows, int columns, int bands,
    ...
    protected long getNextPixelOffset(int band) {
        return this.headerOffset + FloatBSQFileReader.TYPESIZE
            * (band * this.rowsNumber * this.columnsNumber + this.position);
    }

    /** {@inheritDoc} */
    @Override
    protected List<Float> readPixel() throws IOException {
        List<Float> pixel = new ArrayList<Float>();
        if (this.buffers.get(0).remaining() < FloatBSQFileReader.TYPESIZE) {
            this.fillBuffer();
        }
        for (int i = 0; i < this.bandsNumber; i++) {
            pixel.add(this.buffers.get(i).getFloat());
        }
        return pixel;
    }
    ...
    protected int bytesToRead() {
        return (int) Math.min(bufferSize,
            (rowsNumber * columnsNumber - position) * TYPESIZE);
