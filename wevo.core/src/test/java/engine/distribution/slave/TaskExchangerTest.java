...
public class TaskExchangerTest {
  ...
  private final Population<DummyIndividual> transferredPopulation =
      new Population<DummyIndividual>() {
  ...
  private final EvaluationTask<DummyIndividual> transferredTask =
      new EvaluationTask<DummyIndividual>(transferredPopulation);

  /** Evaluation result returned to the server. */
  private final EvaluationResult<DummyIndividual> returnedResult =
      new EvaluationResult<DummyIndividual>(
          new HashMap<DummyIndividual, List<Double>>());
   ...
   */
  @Test
  public void testGettingTaskAtOnce()
      throws IOException, ClassNotFoundException {

    setUpCommonGettingBehavior();

    EasyMock.expect(connectionMock.getResponseCode())
        .andReturn(HttpURLConnection.HTTP_OK);
    EasyMock.expect(connectionMock.getInputStream())
        .andReturn(null);
    EasyMock.expect(taskSerializerMock.deserialize(null))
        .andReturn(transferredTask);

    mockControl.replay();
    EvaluationTask<DummyIndividual> actualTask =
   ...
   */
  @Test
  public void testGettingPopulationWithFewRejects()
      throws IOException, ClassNotFoundException {
    ...
    // MagicNumber off
    EasyMock.expect(connectionMock.getResponseCode())
        .andReturn(HttpURLConnection.HTTP_NO_CONTENT)
        .times(5);
    // MagicNumber on
    EasyMock.expect(connectionMock.getResponseCode())
        .andReturn(HttpURLConnection.HTTP_OK);
  ...
  public void testGettingPopulationWithoutSuccess()
      throws IOException, ClassNotFoundException {

    setUpCommonGettingBehavior();

    // Note that HTTP_UNAVAILABLE *must not* be a part
    // of the protocol for this test to succeed.
    EasyMock.expect(connectionMock.getResponseCode())
        .andReturn(HttpURLConnection.HTTP_UNAVAILABLE)
        .anyTimes();
    EasyMock.expect(connectionMock.getResponseMessage())
        .andReturn("")
        .anyTimes();
  ...
  public void testSendingPopulation() throws IOException {
    ...
    connectionMock.setRequestProperty("Content-Type",
    ...
    connectionMock.connect();
    EasyMock.expect(connectionMock.getOutputStream())
        .andReturn(null);
  ...
  private void setUp() {
    ...
    exchanger = new TaskExchanger<DummyIndividual>(
        taskSerializerMock, resultSerializerMock, urlMock);
  }
}
